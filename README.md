...
**顶栏**：`导出 ▾`（Markdown / 思维导图 / HTML，浏览器下载）· `撤销` · `帮助` · `设置` · `刷新`，并显示当前模型。「设置」分两个页签：**模型设置**（API 端点 / 密钥 / 模型 / 上下文长度 / 思考模式 / 单价 / 预算）与**笔记风格**（风格的新建 / 编辑 / 复制 / 删除 / 恢复默认；格式要求由程序自动附加，这里只写内容与结构要求）。
...
**并发与等待**：需要 LLM 的操作（导入 / 提问 / 批注 / AI 改写 / 按风格重写）同一时刻只允许一个；生成期间**其余操作照常可用**——切换/浏览会话、查看笔记、撤销、导出、改配置等都不会被卡住。LLM 任务进行中再发起第二个 LLM 操作会明确提示「已有 LLM 任务在运行，请稍候或先点停止」；此时切换会话会放弃未写入的回答（界面会先确认），而**导入结果若遇到会话已切换会自动另存为新会话**，不会丢数据。
