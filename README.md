...
- **导出**：一键下载 Markdown / 思维导图（markmap）/ 自包含 HTML（KaTeX 公式渲染，含只读批注弹窗）；会话也可导出为单个 `.json`（CLI `load` 可直接打开）或整库备份（会话 + 知识库 + 置顶），在另一台电脑的 Web 端「导入」即可迁移
...
- **会话历史**：顶部「＋ 新会话」「导入」「导出全部」；点条目加载；右键置顶 / 重命名 / 导出会话 / 删除。
- **迁移 / 备份**：「导出全部」下载整库备份（全部会话 + 知识库 + 置顶，不含 API Key）；在另一台电脑点「导入」选择该 `.json`（或单个会话 JSON）即可。编号冲突**保留两者**并自动加后缀改名（置顶会映射到新编号）、完全相同的会话自动跳过（重复导入不会翻倍）、跨机失效的笔记导出路径自动清空；导入只增量合并，不覆盖本机已有会话与知识库。
- **已读论文 / 已学概念**：点击查看详情；「加载该会话」可跳回当时的问答（若该问答来自批注，会滚动到原文并打开批注弹窗）；右键置顶 / 删除。
- **批量操作**：三个列表均支持 **Ctrl(⌘) 点选、Shift 连选**，右键即可批量置顶 / 导出 / 删除；标题旁显示「已选 N」，`Esc` 取消选择。
