...
**运行中与进度**：所有会调 LLM 的操作（导入 / 提问 / 批注 / 总结 / AI 改写 / 配置测试）都有进度提示与「停止」——导入等命令用标签栏的流动进度条 + `⏹ 停止`；批注弹窗、笔记编辑弹窗、配置测试弹窗内各有一条迷你进度条 + 「停止」。等待首个 token 期间显示**已发送的上下文规模 + 计时**（如「思考中…（上下文约 12k token）· 8s」）；推理模型的**思考过程**（`reasoning_content`）会流式显示在可折叠的「思考过程」区，CLI 下以暗色打印。「停止」点击立即中止（后端打断 + 断开本地流）。
