...
- **原文（PDF）阅读器**：顶栏「**原文**」标签（当前会话导入了 PDF 时才出现）打开内置 [PDF.js](https://mozilla.github.io/pdf.js/)（随程序离线可用），支持翻页 / 页码跳转 / 缩放（滑条 + ±按钮 + Ctrl+滚轮，比例全局记忆）/ 左侧可伸缩折叠目录 / 连续滚动，当前阅读页有高亮标记。**可直接在原文上选中文字提问**：系统把选中内容与**该页截图**一起交给多模态模型，图文对照更准；选中内容会存成批注、原文高亮，点击高亮即可回看。仅针对当前会话导入的那份 PDF
...
**原文（PDF）阅读**：顶栏「**原文**」标签（当前会话导入了 PDF 时才出现）打开阅读器，支持连续滚动、上一页 / 下一页、页码直接跳转、缩放（**滑条** + **± 按钮** + **Ctrl+滚轮**，比例全局记忆，可一键「贴合宽度」）与**左侧目录栏**（`目录` 按钮显示 / 隐藏，可拖动调宽；PDF 没有内置书签时明确提示「此 PDF 没有内置目录」）。当前阅读页有高亮标记，「提问本页」会写明是第几页；在原文里选中文字 → 浮出的「提问」会把选中内容与**当前页截图**一起交给多模态模型，问题与回答仍按批注保存、选中处持久高亮，点击高亮回看；翻页 / 跳页不会打断正在进行的任务。若只想读原文、不需要笔记，导入时选「**PDF → 仅阅读（不生成笔记）**」：不调模型、不生成笔记，直接进入原文视图（扫描件也能看，提问时发送该页截图；若抽不到文字会提示这可能是扫描/图片版，可改用 OCR 导入）。
...
> **变更（0.14.0）**：批注弹窗里的对话现在可以**折叠**——每个节点左侧多了 **▼ / ▶ 箭头**，点一下就把该节点连同它的子树收成**窄窄一行**（只露开头一小段文字 + 省略号，左侧对话树同步收起），再点即可展开；在**正文节点上右键**也有「折叠该节点及子树 / 展开该节点及子树」菜单项。对话很长时用它快速收起不关心的部分。

> **变更（0.15.0）**：修复批注弹窗里**点击左侧对话树空白处没有反应**的问题——之前只有点在「树」本身那点高度才生效，点标题、说明文字或树下方的大片空白都没反应。现在点击侧栏**任何空白处**都会进入「新开一条对话」状态，下一次提问会作为一条新的对话根。

> **变更（0.16.0）**：会话列表的**排序时间**改为「**最后一次修改笔记内容**」的时间——之前只要点开/切换某个会话，它的时间就会被刷新、跳到列表最前，纯浏览也会打乱顺序。现在提问、AI 改写、编辑/增删笔记等**真正的内容改动**才会更新顺序；切换会话、重命名、退出、手动保存都不再影响排序。手动编辑笔记现在也会**立即保存**（之前要等切换或退出才落盘）。

> **变更（0.17.0）**：集中修整 **PDF 原文阅读**体验。**修复切换会话后原文仍是上一份 PDF**（严重）——之前 `/api/pdf/file` 对所有会话用同一地址且被浏览器缓存，现在按会话 / 文件区分并禁用缓存；**「原文」改为顶栏独立标签**（原来和「笔记 / 原文」内嵌小切换条重复、易混）；**缩放改为滑条 + ± 按钮 + Ctrl+滚轮**（不再是几个固定档位，比例全局记忆，可一键「贴合宽度」）；**目录移到左侧栏**（可拖动调宽、可折叠，PDF 无内置书签时明确提示「此 PDF 没有内置目录」而非空白条）；**仅阅读导入弹窗不再多问文件名**（之前会显示用不上的笔记加密字段）；**当前阅读页高亮标记**，「提问本页」写明是第几页。另修了一个通用样式兜底：带 `hidden` 类的元素现在一定隐藏（此前个别按钮因此「点了没反应」）。
