...
PaperHelper 命令：
  ...
  exit                     退出";
    ...
    async fn cmd_ingest(&mut self, rest: &str) -> Result<()> {
        ...
        let prompt = format!(
            "请阅读以下论文全文，按固定四段架构生成一份学习笔记 Markdown。\n\
             架构（每个小标题作为一级 `##`，标题下的全部内容归为同一块，不要再用子标题拆碎）：\n\n\
             ## 一、要解决的问题\n简要说明本文要解决什么问题、为什么重要。\n\n\
             ## 二、前人方案及其不足\n列出已有解决方案，重点说各自的不足（正因为有不足才提出新方案）。\n\n\
             ## 三、本文方案及其优点\n说明作者提出的方案、做法，重点说优点（可含关键公式，用 $$...$$ 包裹）。\n\n\
             ## 四、前景与发展方向\n之后可能的改进或延伸方向。\n\n\
             要求：\n\
             - 第一行用 `# 论文标题` 写出论文标题。\n\
             - 严格按上面四个 `##` 小标题组织；每个小标题下的段落和公式合并为一段，不要为每句话单独成块。\n\
             - 公式用 `$$...$$` 包裹，直接嵌在所属段落里，不要用空行把公式和段落隔开。\n\
             - 简洁清晰，不编造，不要输出额外说明，直接给 Markdown。\n\n\
             论文全文：\n{raw_text}"
        );
        let msgs = vec![
